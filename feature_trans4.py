...
def train_test(num_rows=None):
    # load csv
    train_df = pd.read_csv(data_dir+'train.csv', index_col=['card_id'], nrows=num_rows)
    test_df = pd.read_csv(data_dir+'test.csv', index_col=['card_id'], nrows=num_rows)
...
def historical_transactions(num_rows=None):
    # load csv
    hist_df = pd.read_csv(data_dir+'historical_transactions.csv', nrows=num_rows)
...
def new_merchant_transactions(num_rows=None):
    # load csv
    new_merchant_df = pd.read_csv(data_dir+'new_merchant_transactions.csv', nrows=num_rows)
...
def kfold_lightgbm(train_df, test_df, num_folds, stratified=False, debug=False):
    ...
    if not debug:
        ...
        test_df[['card_id', 'target']].to_csv("/data/dev/lm/elo/out/lgb_goss0129.csv", index=False)
...
def main(debug=False):
    ...
    with timer("split train & test"):
        ...
        train_df.to_csv(out_dir+'train_trans0129.csv',index=True)
        test_df.to_csv(out_dir+'test_trans0129.csv',index=True)
        del df
        gc.collect()
